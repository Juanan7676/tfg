...
En esta sección, como ya adelantamos, veremos que el esquema de firma $LD-OTS$ es, al menos, tan seguro como la función hash utilizada: esto es, veremos que si existe un ataque que rompe el sistema $LD-OTS$, entonces podremos construir un algoritmo que encuentra colisiones de la función hash correspondiente. Antes de ello, necesitamos precisar el concepto de \textit{resistencia a la colisión} de una función hash un poco más \cite{hashfunctions}.
...
\bdefinicion
Sea $\mathcal{G}=(\mathcal{X},\mathcal{Y},\mathcal{K},\mathcal{H})$ una familia de funciones hash, y sean $t,\varepsilon > 0$. Se dice que la familia $\mathcal{G}$ es $(t,\varepsilon)$ \textit{resistente a la colisión} si para todo adversario \[\textsc{Adv} : \mathcal{K} \to \left( \mathcal{X} \times \mathcal{X} \right) \cup \{ error \} \] que dado $K\in \mathcal{K}$, devuelve $x,x'\in\mathcal{X}$ tales que $x\neq x'$ y $h_K(x)=h_K(x')$ o \textit{error} y que puede ejecutarse en tiempo $t$, verifica que
\[ Pr\left[ k \xleftarrow{\$} \mathcal{K},(x,x') \xleftarrow{\$} \textsc{Adv}(k) : x \neq x' \wedge g_k(x) = g_k(x') \right] \leq \varepsilon \]
\edefinicion
