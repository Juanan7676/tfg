...
Nuestro objetivo a partir de ahora será estudiar los dos algoritmos cuánticos que ponen en peligro una gran parte de la criptografía usada en la actualidad. Antes de ello, debemos pararnos a estudiar un concepto vital en el desarrollo de la teoría de ordenadores cuánticos: los espacios de Hilbert, que se apoyan en los productos escalares.

\section{Definiciones básicas}
En este desarrollo, consideraremos en todo momento que el espacio vectorial subyacente $X$ es de dimensión finita, es decir, $\dim X = n \in \mathbb{N}$; ya que no trabajaremos en los capítulos siguientes con espacios de Hilbert de dimensión infinita. Esto simplificará algunas demostraciones aquí presentadas; no obstante, con un poco más de esfuerzo, se pueden hacer para el caso general.
...
Ya que los espacios de Hilbert son espacios vectoriales, podemos hablar de aplicaciones lineales entre ellos y de formas lineales. De entre ellas, en el contexto de la computación cuántica nos interesarán aquellas que sean \textit{unitarias}. Para poder definirlas correctamente, necesitamos un poco de teoría de espacios de Hilbert.

\blema
	Sean $X,Y$ espacios de Hilbert y $f : X \to Y$ una aplicación lineal. Entonces, $f$ es continua si y sólo si $f(S_X) = f(\{ x\in X : \| x \| = 1 \}) $ es acotado.
\elema

Una demostración de este hecho puede consultarse en \cite{afuncional}.
...
\begin{proof} \hfill \\ \\
		...
		Dado $\varepsilon>0$, tomando $\delta_2>0$ de modo que $\delta_2 < \frac{\varepsilon}{2M\| a \|}$ si $a \neq 0$ y $\delta_2 < \frac{\varepsilon}{2M(||b||+\delta_2)}$, tenemos que si $\| x-a \| < \delta_1$ y $\|y-b\| < \delta_2$, entonces $\|u(x,y)-u(a,b)\|<\varepsilon$, lo que queríamos probar. Si fuese $a=0$, entonces el resultado es trivial tomando $\delta_1 = \frac{ \varepsilon}{\| y \|}$.
...
\bproposicion
\label{prop:unitaria}
	Sea $X$ un espacio de Hilbert y sea $A : X \to X$ una aplicación lineal y continua. Si $A$ es unitaria, entonces para todo $x,y\in X$, $\langle A(x),A(y) \rangle = \langle x,y \rangle $. En particular, $\| A(x) \| = \| x \|$ para todo $x\in X$.
\eproposicion

\begin{proof}
	Para todo $x,y \in X$, $\langle A(x),A(y) \rangle = \langle x,A'(A(y)) \rangle $ por definición de operador adjunto, y como es unitario, $A'A=Id$ y tenemos que $\langle A(x),A(y) \rangle = \langle x,y \rangle$.
	
	El segundo resultado se desprende de la definición de norma: $ \| A(x) \| = \sqrt{\langle A(x),A(x) \rangle} = \sqrt{\langle x,x \rangle} = \| x \| $.
\end{proof}
